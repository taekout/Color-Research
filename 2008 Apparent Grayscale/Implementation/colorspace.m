...
%                 -> It's been modified by Jack Shin. It uses just normal
%                 RGB........
...
% I ll add some code below instead of the code removed just above.
SrcT = SrcSpace;
DestT = DestSpace;
...
% I modified it... it just uses RGB instead of R'G'B'.
switch SrcSpace
case 'rgb'
   return;
case 'hsv'
...
otherwise   % Convert from some gamma-corrected space
   ...
   % I've removed the gamma correction code. instead, i will add the below.
   R = Image(:,:,1);
   G = Image(:,:,2);
   B = Image(:,:,3);
   %%% Convert RGB to XYZ
