...
P_input = input('Please enter P value : ');
disp('Now processing ...');
...
Grey_Final = GreyImage + Recursive_f(originalImage, GreyImage, Depth, K, P_input);
...
tmp_filename = strcat(FileName, '_Grey_', num2str(P_input), '_');
for i = 1 : Depth
    tmp_filename = strcat(tmp_filename, num2str(K(i)));
end
tmp_filename = strcat(tmp_filename, '_MAIN3.bmp');
imwrite(GreyImage, tmp_filename, 'bmp');
tmp_filename = strcat(FileName, '_Int_', num2str(P_input), '_');
for i = 1 : Depth
    tmp_filename = strcat(tmp_filename, num2str(K(i)));
end
tmp_filename = strcat(tmp_filename, '_MAIN3.bmp');
...
tmp =  (Hn_G(:,:,1) .* K(nMaximumDepth - N) .* delta) ;
