//AAG edited to fit image loading
//
...
const char *inputfilename;
  Fl_Window	*FBwindow;// Main window
  ...
  void CreateFileBrowser()
{
  ...
  FBwindow = new Fl_Window(400, 200, "File Chooser Test");

  filter = new Fl_Input(50, 10, 315, 25, "Filter:");

  filter->value("Img Files (*.ppm)\t");
            ...
            void            *data)	// I - Data
{
  ...
  inputfilename = fc->value();

  printf("    filename = \"%s\"\n", inputfilename ? inputfilename : "(null)");
  inputfilename = fl_filename_name(inputfilename);
  printf("    filename = \"%s\"\n", inputfilename ? inputfilename : "(null)");

  extern Color2GrayIt *c2g;
  strcpy(c2g->fname, inputfilename);  
  c2g->SourceLoaded = 0;
  close_callback();
}
