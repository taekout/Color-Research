function [tones,recolor] = ColorReordering(picture,effect,scale,noise)
% Created: 4 June 2005
...
scalechrominance=155.56349186104045536818575966307;
...
LABimage = applycform(picture, c);
LABimage(find(LABimage(:,2)>110))=110;
LABimage(find(LABimage(:,3)>110))=110;
LABimage(find(LABimage(:,2)<-110))= -110;
LABimage(find(LABimage(:,3)<-110))= -110; %%%%%%%%%%%%% Think more about this!
original=image;
...
delta=LABimage-LABimage(look,:);
contrastchange=abs(delta(:,1)); % delta Y
contrastdirection=sign(delta(:,1)); % the sign of delta Y
%colordifference=delta(2, :); % delta D
colordifference=delta(:,2:end);
colordifference=sqrt(sum(colordifference.*colordifference,2))+eps;
colordifference=im2double(colordifference)./scalechrominance;
...
% Derive a chromatic axis from the weighted sum of chromatic differences
% between paired pixels
contrastchange=im2double(contrastchange) ./ 100;
weight=1-((contrastchange)./colordifference);
